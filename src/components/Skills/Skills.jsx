...
function Skills(props) {
  ...
  const { size, opacity, ...rest } = useSpring({
    ref: springRef,
    config: config.stiff,
    from: { size: '20%', background: props.theme.color.main.normal },
    to: { size: open ? '100%' : '20%', background: open ? 'lightgray' : props.theme.color.main.normal }
  })
  ...
  return (
    <Element name="skills">
      <MyContainer>
        <SkillsBox style={{ ...rest, width: size, height: size }} onClick={() => setOpen(open => !open)}>
